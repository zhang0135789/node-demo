...
async function migrateDbSync(connection) {
	...
	if (version < 6) {
		arrQueries.push(arrQueries, "CREATE TABLE IF NOT EXISTS chat_messages ( \n\
		...
		arrQueries.push(arrQueries, "CREATE INDEX IF NOT EXISTS chatMessagesIndexByDeviceAddress ON chat_messages(correspondent_address, id)");
	...
	if (version < 18) {
		arrQueries.push("CREATE TABLE IF NOT EXISTS attested_fields ( \n\
						unit CHAR(44) NOT NULL, \n\
		...
		arrQueries.push("CREATE INDEX IF NOT EXISTS attestedFieldsByAddressField ON attested_fields(address, `field`)");
		arrQueries.push("CREATE TABLE IF NOT EXISTS original_addresses ( \n\
						unit CHAR(44) NOT NULL, \n\
						address CHAR(32) NOT NULL, \n\
		...
		let rows = await db.toList("SELECT unit, message_index, attestor_address, address, payload FROM attestations CROSS JOIN messages USING(unit, message_index)");
	...
	if (version < 20)
		arrQueries.push("DELETE FROM known_bad_joints");
	if (version < 21)
		arrQueries.push("ALTER TABLE push_registrations ADD COLUMN platform TEXT NOT NULL DEFAULT 'android'");
