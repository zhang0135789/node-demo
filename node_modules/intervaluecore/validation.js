...
async function validateForJoint(objJoint, callbacks) {
		...
		async.series(
			...
			function (err) {
				...
				if (err) {
					...
					profiler.start();
					conn.query("COMMIT", async function () {
						...
						if (objJoint.unsigned) {
							...
							await callbacks.ifOk(objValidationState, unlock);
					});
				}
			}
		); // async.series
...
exports.validateForJoint = validateForJoint;
